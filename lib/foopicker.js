...
export const FooPicker = (function () {
  "use strict";
  ...
  function FooPicker() {
    ...
    if (arguments[0] && typeof arguments[0] === "object") {
      ...
      _pickerField = _self.options.pickerField;
      _ctx = _self.options.ctx;
    }

    // Show date picker on click
    _self.showPicker = function () {
      _self.buildPicker();
      var pickerDiv = _self.options.ctx.shadowRoot.getElementById(
        "foopicker-" + _id
      );
      var pickerField = _self.options.pickerField;
      if (pickerField) {
    ...
    _self.hidePicker = function (event) {
      setTimeout(function () {
        ...
        if (!_self.monthChange && !_self.isPickerClicked) {
          ...
          pickerDiv = _self.options.ctx.shadowRoot.getElementById(
            "foopicker-" + _id
          );
          ...
          pickerField = _self.options.pickerField;
          if (pickerField) {
            pickerField.focus();
          }
    ...
    _self.selectDate = function (event) {
      ...
      // var el = _self.options.ctx.shadowRoot.getElementById(event.target.id);
      var el = event.target;
      var pickerField = _self.options.pickerField;
      if (el) {
    ...
    _self.buildPicker = function () {
      var pickerDiv = this.options.ctx.shadowRoot.getElementById(
        "foopicker-" + _id
      );
      var pickerField = this.options.pickerField;
    ...
    _self.buildTemplate = function () {
      ...
      this.options.ctx.shadowRoot.appendChild(pickerDiv);
      addListeners(_self);
    };
  ...
  var Calendar = {
    ...
    addListeners: function (instance) {
      ...
      var pickerDiv = instance.options.ctx.shadowRoot.getElementById(
        "foopicker-" + id
      );
      if (pickerDiv) {
        ...
        for (var count = 0; count < el.length; count++) {
          if (typeof el[count].onclick !== "function") {
            if (
              ...
              var elem = instance.options.ctx.shadowRoot.querySelector(
                "[data-day='" + (count + 1) + "']"
              );
              //var elem = document.getElementById(
              //  id + "-foopicker__day--" + (count + 1)
  ...
  function addListeners(picker) {
    var el = picker.options.pickerField; // document.getElementById(picker.options.id);
    if (el) {
