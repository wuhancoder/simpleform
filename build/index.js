...
class MyInput extends MyComponent {
  ...
  applyValidationRule(rule) {
    console.log(JSON.stringify(rule)); //this.inputEl = this.shadowRoot.querySelector("input");
  ...
  firstUpdated(changedProperties) {
    ...
    if (typeof this.validations !== "undefined" && this.validations !== "undefined") {
    ...
    if (this.myvalidation) {
      this.inputEl.required = this.myvalidation && this.myvalidation.localeCompare("true") == 0;
    }
  }

  validate() {
    //this.inputEl = this.shadowRoot.querySelector("input");
    this.value = this.inputEl.value; //if (!this.inputEl) return;

    console.log(this.inputEl + " this.....   " + this.value);

    if (!this.inputEl.checkValidity()) {
      console.log(this.inputEl.validationMessage);
      this.shadowRoot.querySelector(".errors").textContent = this.inputEl.validationMessage;
    } else {
      this.resetErrors();
    }
  }
  ...
  render() {
    return html`
      ...
      <div class="input-container">
        ${typeof this.icon === "undefined" || this.icon === "undefined" ? html`<label for="${this.name}">${this.label}</label>` : html`<i class="material-icons icon">${this.icon}</i>`}
        <input
          type="${this.type}"
          id="${this.name}"
...
customElements.define("my-input", MyInput); //?required=${this.myvalidation &&
...
class MyDropdown extends MyComponent {
  ...
  renderOptions() {
    if (typeof this.datasets === "undefined" || this.datasets == "undefined") return html``;else return html`
        ${this.datasets.map(i => html`<option value=${i.value}>${i.option}</option>`)};
      `;
  }
...
customElements.define("my-dropdown", MyDropdown); //?required=${this.myvalidation &&
...
class MyCheckbox extends MyComponent {
  ...
  get value() {
    return [...this.shadowRoot.querySelectorAll("input[type=checkbox]:checked")].map(i => {
      return i.value;
    }).join(", ");
  }
  ...
  handleChange() {
    ...
    //   })
    //   .join(", ");
    let checked = [...this.parentElement.parentElement.querySelectorAll("input[type=checkbox]:checked")].map(i => {
      return i.value;
    }).join(", ");
  ...
  firstUpdated(changedProperties) {
    super.firstUpdated(changedProperties);
    this.inputEl = this.shadowRoot.querySelector("input"); // [...this.shadowRoot.querySelectorAll("input[type=checkbox]")].map((i) =>
...
customElements.define("my-checkbox", MyCheckbox); //?required=${this.myvalidation &&

class MyRadiobutton extends MyComponent {
  static get styles() {
    return css`
      fieldset {
        background-color: powderblue;
  ...
  get value() {
    return this.shadowRoot.querySelectorAll("input[type=radio]:checked")[0].value;
  }
...
class MySelect extends MyComponent {
  ...
  firstUpdated(changedProperties) {
    super.firstUpdated(changedProperties);
    this.inputEl = this.shadowRoot.querySelector("my-dropdown, my-checkbox,my-radio");
  }
...
customElements.define("my-select", MySelect); //?required=${this.myvalidation &&
...
class MyForm extends LitElement {
  ...
  async readConfig() {
    fetch(this.config).then(res => res.json()).then(data => {
  ...
  renderInput(i) {
    if (i.input === "text") return html` <div class="row">
        <my-input
          name="${i.name}"
          label="${i.label}"
          icon="${i.icon}"
  ...
  submitForm(e) {
    let allInputs = [...this.shadowRoot.querySelectorAll("my-input, my-select")];
